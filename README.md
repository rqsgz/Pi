...
## 概览
NeuroLux 把来自 ThinkGear（TGAM）设备的脑电数据通过解析 → 信号处理 → 分类 → 映射 → 执行（WiZ / GPIO）→ 广播 / 仪表盘 的流水线，实时驱动灯光并支持回放与诊断。
...
本节为附加内容：在树莓派与 WiZ 灯泡不在同一物理网络（例如树莓派连手机热点、灯泡连家中 Wi‑Fi）时，如何用 Tailscale 实现远程局域网控制的原理、配置与示例。
