...
## Tailscale 与 WiZ 灯泡 — 简要总结

### 核心原理
Tailscale 基于 WireGuard 建立加密的 overlay/VPN，把物理上不同网络的设备连成同一逻辑网络（每台设备分配 100.x.x.x 虚拟 IP）。流量优先点对点穿透，穿透失败时通过加密中继（DERP），数据仍为端到端加密。
...
- 需要在广告子网的设备上启用 IP 转发（Linux: `sysctl net.ipv4.ip_forward=1`）并配置相应防火墙/转发规则。
...
- 启用路由时配置防火墙以限制非必要流量；DERP 中继会增加延迟但数据仍被加密。

### 常见排查步骤
- 在两端运行 `tailscale status`，确认节点与路由是否可见。
- 在广告子网设备上确认 IP 转发与防火墙规则。
